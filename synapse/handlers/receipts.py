...
class ReceiptsHandler:
    ...
    async def received_client_receipt(
        ...
        if not await self.event_handler.get_event(user_id, room_id, event_id):
            return
        logger.info(f"broadcast :read receipt: {user_id}")
        ...
        # EIMIS broadcast POC
        other_mx = self.get_user_other_mxid(user_id.to_string())
        if other_mx:
            logger.info(f"  broadcast: to {other_mx}")
            ...
            await self._handle_new_receipts([receipt2])
            if self.federation_sender and receipt_type != ReceiptTypes.READ_PRIVATE:
                logger.info(f"  broadcast : federation send {other_mx}")
                await self.federation_sender.send_read_receipt(receipt2)
    ...
    def get_user_other_mxid(self, user_id):
        ...
        if 'admin' in localpart:
            return ''
        other_server = self.get_other_server()
        if other_server:
            return f"@{localpart}:{other_server}"
        return ''
